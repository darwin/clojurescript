...
#?(:clj
   (defn- prepare-source-mapping-url [^File sm-file source-map-asset-path source-map-url output-dir timestamp]
     (let [mapping-url (if source-map-asset-path
                         (str source-map-asset-path "/" (strip-prefix-path (url-path sm-file) output-dir))
...
#?(:clj
   (defn- prepare-source-mapping-data-url [^String sm-json]
     (let [encoded-json (.encodeToString (Base64/getEncoder) (.getBytes sm-json "UTF-8"))]
...
#?(:clj
   (defn emit-source-map [src dest sm-data opts]
     (let [sm-file (io/file (str (.getPath ^File dest) ".map"))
           ...
           src-path (url-path src)
           sm-json (sm/encode {src-path (:source-map sm-data)}
                              {:lines                   (+ (:gen-line sm-data) 2)
                               :file                    (url-path dest)
                               :source-map-path         (:source-map-path opts)
                               :source-map-timestamp    (:source-map-timestamp opts)
                               :source-map-pretty-print (:source-map-pretty-print opts)
                               :relpaths                {src-path (util/ns->relpath (first (:provides opts)) (:ext opts))}
           ...
           source-mapping-url (if inline-source-maps?
                                ...
                                (let [source-map-asset-path (:source-map-asset-path opts)
                                      ...
                                      output-dir (url-path (.getAbsoluteFile (io/file (:output-dir opts))))
...
#?(:clj
   (defn prepare-cached-core-if-needed [ns ext opts]
     (if-not (:inline-source-maps opts)
          ...
          (fn []
            ...
            (util/measure (and (or ana/*verbose* (:verbose opts))
              ...
              (let [ext (util/ext src)
               ...
               (if-let [cached (prepare-cached-core-if-needed ns ext opts)]
